...
def get_info_fondo(fondo,link):
    ...
    web_fondo1=requests.get(link+'&vista=1')
...
def xml_scrap(xml_soup, nif, fondo):
    ...
    try:context_ref=soup.find("xbrli:context")['id']
...
def xml_scrap_clase(xml_soup, nif, fondo, i):
    ...
    for j in range(len(float_vars)):
        ...
        if fvar != None:
            try:
                float_vars[j] = float(fvar[i].text)
            except:
                pass
    ...
    for j in range(len(int_vars)):
        ...
        if ivar != None:
            try:
                int_vars[j] = int(float(ivar[i].text))
            except:
                pass
    n_participes, patrimonio = int_vars
