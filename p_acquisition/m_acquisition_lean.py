...
def get_info_fondo(fondo,link):
    ...
    if type(temp)!= bool:  #exportado
        temp.drop_duplicates(['name','period']).to_csv(f'data/csv/{clean_fondo}.csv', sep='*', index=False)
...
def xml_scrap(xml_soup, nif, fondo):
    ...
    context_ref_nodash = f'FIM_{period_type}{period_number}{start_date.year}_V{nif}_da'
    context_ref2_nodash = f'FIM_{period_type}{period_number}{start_date.year}_V{nif}_ia'
    context_ref3_nodash = f'FIM_{period_type}{period_number}{start_date.year}_V{nif}_daq'
    ...
    return {"fondo": fondo, "name": name, "period": full_period,
            'start_date': start_date, 'end_date': end_date,  'riesgo': riesgo,
...
def xml_scrap_clase(xml_soup, nif, fondo, i):
    ...
    try:remun_liq = float(remun_liq.text)
    except: pass
    ...
    return {"fondo": fondo, "name": class_name, "period": full_period,
            'start_date': start_date, 'end_date': end_date, 'riesgo': riesgo,
            'rotacion': rotacion, 'rentab_avg': rentab_avg, 'remun_liq':remun_liq,
            'n_participaciones': n_participaciones, 'n_participes': n_participes,
